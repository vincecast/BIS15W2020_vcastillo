...
## `rename()`
The `rename()` function is actually part of *dplyr*, but I put it here because I think of it as part of transforming untidy data. Remember, you can still rename columns as part of `select()`.
```{r}
tb_data %>% 
...
msleep %>%
  group_by(order) %>%
  summarize(meanbrainwt=mean(brainwt))
```
...
msleep %>%
  group_by(order) %>%
  summarize(meanbrainwt=mean(brainwt, na.rm=T))
```
