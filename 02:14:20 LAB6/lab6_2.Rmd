...
homerange %>% 
  ...
  geom_histogram(alpha = 1, color = "magenta")+
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_density(fill="pink", alpha  =1, color = "magenta")+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x=log10.mass)) +
  geom_histogram(aes(y = ..density..), binwidth = .4, fill = "yellow", alpha = 0.8, color = "skyblue")+
...
homerange <- homerange %>%
  mutate(range_category = case_when(log10.hra <= 3.653 ~ "very small",
                                    ...
                                    log10.hra > 4.595 & log10.hra <= 6.1016 ~ "medium",
...
```{r}
homerange %>%
  select(range_category, log10.mass, taxon) %>%
